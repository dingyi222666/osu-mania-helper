...
import { find } from './findPatterns'
import { calculateClusteredPatterns } from './clustering'
import { CORE_PATTERN_LIST } from './patternsDef'
import { lnPercent, svTime } from './primitives'
import { NoteType } from './types'
import { categoriseChart } from './categorise'
import type { Chart, PatternCluster, PatternReport, ModeTag, CorePatternName } from './types'

const LN_CORE_PATTERNS = new Set<string>(['Coordination', 'Density', 'Wildcard'])

function hbRowRatio(chart: Chart): number {
...
function resolveModeTag(lnRatio: number, hbRatio: number): ModeTag {
...
export function fromChart(chart: Chart): PatternReport {
  ...
  function canBePruned(cluster: PatternCluster): boolean {
  ...
  const prunedClusters: PatternCluster[] = []
  ...
  return {
    ...
    get ImportantClusters(): PatternCluster[] {
