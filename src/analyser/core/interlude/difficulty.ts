...
import { calculateNoteRatings } from './noteDifficulty'
import { calculateVariety } from './variety'
import { calculateFingerStrains, calculateHandStrains } from './strain'
import type { NoteRow, InterludeDifficultyResult } from './types'

const CURVE_POWER = f32(0.6)
const CURVE_SCALE = f32(0.4056)
const MOST_IMPORTANT_NOTES = f32(2500.0)

function weightingCurve(x: number): number {
  return f32(0.002 + Math.pow(x, 4.0))
}

export function weightedOverallDifficulty(data: Iterable<number>): number {
...
export function calculateInterludeDifficulty(rate: number, noteRows: NoteRow[]): InterludeDifficultyResult {
