...
import type { NoteRow, NoteDifficultyItem } from './types'
...
function jackCompensation(jackDelta: number, streamDelta: number): number {
...
export function noteDifficultyTotal(note: NoteDifficultyItem): number {
  return f32(
    Math.pow(
...
export function calculateNoteRatings(rate: number, noteRows: NoteRow[]): NoteDifficultyItem[][] {
  ...
  const data: NoteDifficultyItem[][] = Array.from({ length: noteRows.length }, () => (
  ...
  const lastNoteInColumn = new Array<number>(keys).fill(firstTime - 1000000.0)
