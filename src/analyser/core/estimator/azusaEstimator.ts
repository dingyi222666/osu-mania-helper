...
const GREEK_BY_INDEX: readonly string[] = Object.freeze([
  'Alpha',
  'Beta',
  'Gamma',
  'Delta',
...
const RC_TIER_CANDIDATES: readonly { suffix: string; offset: number }[] = Object.freeze([
...
const AZUSA_CALIBRATION_LOW_BLOCKS: readonly [number, number, number][] = Object.freeze([
...
const AZUSA_CALIBRATION_HIGH_BLOCKS: readonly [number, number, number][] = Object.freeze([
...
const AZUSA_ISOTONIC_POINTS: readonly [number, number][] = Object.freeze([
...
function buildErrorResult(code: string, message: string, extras: { lnRatio?: number; columnCount?: number } = {}): AzusaErrorResult {
  return {
    star: Number.NaN,
...
function fmt4(value: number | null | undefined): number | null {
  return Number.isFinite(value as number) ? Number((value as number).toFixed(4)) : null
}

function piecewiseLinear(x: number, knots: readonly (readonly number[])[], valueCol: number = 1): number {
  const v = Number(x)
...
function piecewiseBlock(x: number, blocks: readonly (readonly number[])[]): number {
  const v = Number(x)
...
function formatRcBaseLabel(base: number): string {
  if (base <= 0) {
...
function numericToRcLabel(numeric: number): string {
  ...
  let bestMatch: { base: number; suffix: string; distance: number } | null = null
...
function estimateDanielNumeric(result: EstimatorResult | null): number | null {
  const numericRaw = result?.numericDifficulty
  if (typeof numericRaw === 'number' && Number.isFinite(numericRaw)) {
    return numericRaw
  }

  if (typeof numericRaw === 'string' && (numericRaw as string).trim().length > 0) {
...
function hasDanielNativeNumeric(result: EstimatorResult | null): boolean {
...
function estimateSunnyNumeric(result: EstimatorResult | null): number | null {
  const star = Number(result?.star)
  if (!Number.isFinite(star)) {
    return null
  }
...
function powerMean(values: number[], p: number): number {
  if (!values.length) {
    return 0
  }
...
function buildTapNotes(parsed: { columns: number[]; noteStarts: number[] }): TapNote[] {
  const taps: TapNote[] = []
...
function annotateRows(taps: TapNote[], toleranceMs: number): void {
  if (!taps.length) {
    return
  }
...
function expDecayFactor(dtMs: number, tauMs: number): number {
...
function skillFromStates(states: number[]): number {
  let sum = 0
...
function buildDifficultyCurve(taps: TapNote[]): DifficultyCurve {
  const states = {
  ...
  const density250: number[] = []
  const density500: number[] = []
  const jackRawSeries: number[] = []
  ...
  const chordSeries: number[] = []
  const techSeries: number[] = []
  const jackSeries: number[] = []
  const times: number[] = []
...
function computeAzusaNumericFromCurve(curve: DifficultyCurve, noteCount: number): number {
  ...
  // Chordjack interaction: chord density x jack density co-occurrence
...
function resolveRcBlendComponents(primaryNumeric: number, danielNumeric: number | null, sunnyNumeric: number | null, curveHints: CurveHints | null = null): BlendDetails {
  ...
  const highBase = (() => {
    ...
    const anchorImbalance = Number.isFinite(curveHints?.anchorImbalance as number) ? curveHints!.anchorImbalance! : null
    const chordRate = Number.isFinite(curveHints?.chordRate as number) ? curveHints!.chordRate! : null
    const jackQ95 = Number.isFinite(curveHints?.jackQ95 as number) ? curveHints!.jackQ95! : null
  ...
  if (lowBase == null && highBase == null) {
    return { value: null, lowGateSource, lowGate, highGate, lowBase, highBase }
  }

  if (lowBase == null) {
    return { value: highBase, lowGateSource, lowGate, highGate, lowBase, highBase }
  }

  if (highBase == null) {
    return { value: lowBase + lowLift, lowGateSource, lowGate, highGate, lowBase, highBase }
  }
...
function calibrateAzusaNumeric(value: number | null, lowGate: number | null = null, highGate: number | null = null): number {
...
function calibrateAzusaOutputNumeric(value: number): number {
  return piecewiseLinear(Number(value), AZUSA_ISOTONIC_POINTS, 1)
}
function computeCurveGapResidualCorrection(baseNumeric: number, blendDetails: BlendDetails, curveStats: CurveHints, primaryNumeric: number, sunnyNumeric: number | null, danielNumeric: number | null): number {
  ...
  const anchorImbalance = Number.isFinite(curveStats?.anchorImbalance as number) ? curveStats.anchorImbalance! : 0
  const chordRate = Number.isFinite(curveStats?.chordRate as number) ? curveStats.chordRate! : 0
  const jackQ95 = Number.isFinite(curveStats?.jackQ95 as number) ? curveStats.jackQ95! : 0
...
function computeReferenceCorrection(azusaEst: number, danielNumeric: number | null, sunnyNumeric: number | null): number {
  ...
  let gate: number, coeffD: number, coeffS: number
...
export function runAzusaEstimatorFromText(osuText: string, options: AzusaEstimatorOptions = {}): AzusaEstimatorResult | AzusaErrorResult {
  ...
  let danielNumeric: number | null = null
  let danielResult: EstimatorResult | null = precomputedDanielResult || null
  let danielHasNativeNumeric = false
  let sunnyNumeric: number | null = null
  let sunnyResult: EstimatorResult | null = precomputedSunnyResult || null
  ...
  const estDiffLabel = numericToRcLabel(finalNumeric)

  const result: AzusaEstimatorResult = {
    star: Number((3.4 + 0.38 * finalNumeric).toFixed(4)),
    lnRatio,
    columnCount,
    estDiff: estDiffLabel,
