...
import type {
  Corners,
  DanielCalculateOptions,
  DanielNoteEntry,
  DanielPreprocessedData,
  DifficultyGraph,
  ReworkResult,
  ReworkResultObject,
} from './types'
...
function preprocessDaniel(osuText: string, speedRate: number, _odFlag: string | number | null): DanielPreprocessedData {
  ...
  const K = columnCount
  const noteSeqByColumn: DanielNoteEntry[][] = Array.from({ length: K }, () => [])
...
function getCorners(T: number, noteSeq: DanielNoteEntry[]): Corners {
...
function getKeyUsage(K: number, T: number, noteSeq: DanielNoteEntry[], baseCorners: number[]): Record<number, Uint8Array> {
  const keyUsage: Record<number, Uint8Array> = {}
  for (let k = 0; k < K; k += 1) {
...
function getKeyUsage400(K: number, noteSeq: DanielNoteEntry[], baseCorners: number[]): Record<number, Float64Array> {
  const keyUsage400: Record<number, Float64Array> = {}
  for (let k = 0; k < K; k += 1) {
...
function computeAnchor(K: number, keyUsage400: Record<number, Float64Array>, baseCorners: number[]): Float64Array {
  ...
  for (let idx = 0; idx < baseCorners.length; idx += 1) {
    ...
    const nonZero = counts.filter((v: number) => v > 0)
    let raw = 0
    if (nonZero.length > 1) {
      let walk = 0
      let maxWalk = 0
...
function computeJbar(K: number, x: number, noteSeqByColumn: DanielNoteEntry[][], baseCorners: number[]): { deltaKs: Record<number, Float64Array>; Jbar: Float64Array } {
  const jackNerfer = (delta: number): number => 1 - 7e-5 * ((0.15 + Math.abs(delta - 0.08)) ** (-4))

  const Jks: Record<number, Float64Array> = {}
  const deltaKs: Record<number, Float64Array> = {}
  for (let k = 0; k < K; k += 1) {
...
function computeXbar(K: number, x: number, noteSeqByColumn: DanielNoteEntry[][], activeColumns: number[][], baseCorners: number[]): Float64Array {
  const crossMatrix = [
    [-1],
    [0.075, 0.075],
    [0.125, 0.05, 0.125],
  ...
  const Xks: Record<number, Float64Array> = {}
  const fastCross: Record<number, Float64Array> = {}
  ...
  for (let k = 0; k < K + 1; k += 1) {
    let notesInPair: DanielNoteEntry[] = []
    if (k === 0) {
...
function computePbar(x: number, noteSeq: DanielNoteEntry[], anchor: Float64Array, baseCorners: number[]): Float64Array {
...
function computeAbar(K: number, activeColumns: number[][], deltaKs: Record<number, Float64Array>, ACorners: number[], baseCorners: number[]): Float64Array {
  const dks: Record<number, Float64Array> = {}
  for (let k = 0; k < K - 1; k += 1) {
...
function computeCAndKs(K: number, noteSeq: DanielNoteEntry[], keyUsage: Record<number, Uint8Array>, baseCorners: number[]): { CStep: Float64Array; KsStep: Float64Array } {
...
function applyProximityEnvelope(allCorners: number[], DAll: number[], noteSeq: DanielNoteEntry[]): number[] {
  if (!noteSeq.length) {
...
function smoothDForGraph(allCorners: number[], DAll: number[], noteSeq: DanielNoteEntry[]): number[] {
...
export function calculateDaniel(osuText: string, speedRate: number = 1.0, odFlag: string | number | null = null, options: DanielCalculateOptions = {}): ReworkResult {
