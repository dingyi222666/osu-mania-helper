...
import type {
  Corners,
  DifficultyGraph,
  LnSparseRepresentation,
  NoteEntry,
  PreprocessedData,
  ReworkResult,
  ReworkResultObject,
  SunnyCalculateOptions,
} from './types'
...
function cumulativeSum(x: ArrayLike<number>, f: ArrayLike<number>): Float64Array {
...
function queryCumsum(q: number, x: ArrayLike<number>, F: Float64Array, f: ArrayLike<number>): number {
  if (q <= x[0]) return 0
...
function smoothOnCorners(x: number[], f: ArrayLike<number>, window: number, scale: number = 1.0, mode: string = 'sum'): Float64Array {
...
function interpValues(newX: ArrayLike<number>, oldX: ArrayLike<number>, oldVals: ArrayLike<number>): Float64Array {
...
function stepInterp(newX: number[], oldX: number[], oldVals: ArrayLike<number>): Float64Array {
  const out = new Float64Array(newX.length)
  let idx = 0
  for (let i = 0; i < newX.length; i += 1) {
    const x = newX[i]
...
function gaussianFilter1d(data: ArrayLike<number>, sigmaSamples: number): number[] {
...
function mergeByHead(a: NoteEntry[], b: NoteEntry[]): NoteEntry[] {
...
function findNextNoteInColumn(note: NoteEntry, times: number[], noteSeqByColumn: NoteEntry[][]): NoteEntry {
  const k = note[0]
  const h = note[1]
  const idx = bisectLeft(times, h)
  return idx + 1 < noteSeqByColumn[k].length ? noteSeqByColumn[k][idx + 1] : [0, 1e9, 1e9]
}
function preprocessFile(osuText: string, speedRate: number, odFlag: string | number | null, cvtFlag: string | null): PreprocessedData {
  ...
  if (odFlag == null) {
    ...
    od = Number.parseFloat(String(odFlag))
  }

  const timeScale = speedRate !== 0 ? 1 / speedRate : 1

  const noteSeq: NoteEntry[] = []
  ...
  const noteSeqByColumn: NoteEntry[][] = Array.from({ length: K }, () => [])
  ...
  const lnSeqByColumn: NoteEntry[][] = Array.from({ length: K }, () => [])
...
function getCorners(T: number, noteSeq: NoteEntry[]): Corners {
...
function getKeyUsage(K: number, T: number, noteSeq: NoteEntry[], baseCorners: number[]): Record<number, boolean[]> {
  const keyUsage: Record<number, boolean[]> = {}
  for (let k = 0; k < K; k += 1) {
...
function getKeyUsage400(K: number, T: number, noteSeq: NoteEntry[], baseCorners: number[]): Record<number, number[]> {
  const keyUsage400: Record<number, number[]> = {}
  for (let k = 0; k < K; k += 1) {
...
function computeAnchor(K: number, keyUsage400: Record<number, number[]>, baseCorners: number[]): number[] {
  const anchor: number[] = new Array(baseCorners.length).fill(0)
  ...
  for (let idx = 0; idx < baseCorners.length; idx += 1) {
    ...
    const nonZeroCounts = counts.filter((v: number) => v !== 0)
...
function lnBodiesCountSparseRepresentation(lnSeq: NoteEntry[], T: number): LnSparseRepresentation {
...
function lnSum(a: number, b: number, lnRep: LnSparseRepresentation): number {
...
function computeJbar(K: number, x: number, noteSeqByColumn: NoteEntry[][], baseCorners: number[]): { deltaKs: Record<number, number[]>; Jbar: number[] } {
  const jackNerfer = (delta: number): number => 1 - 7e-5 * ((0.15 + Math.abs(delta - 0.08)) ** (-4))

  const Jks: Record<number, number[]> = {}
  const deltaKs: Record<number, number[]> = {}
  for (let k = 0; k < K; k += 1) {
  ...
  const JbarKs: Record<number, Float64Array> = {}
  for (let k = 0; k < K; k += 1) {
    JbarKs[k] = smoothOnCorners(baseCorners, Jks[k], 500, 0.001, 'sum')
  }

  const Jbar: number[] = new Array(baseCorners.length).fill(0)
...
function computeXbar(K: number, x: number, noteSeqByColumn: NoteEntry[][], activeColumns: number[][], baseCorners: number[]): Float64Array {
  const crossMatrix = [
    [-1],
    [0.075, 0.075],
    [0.125, 0.05, 0.125],
  ...
  const Xks: Record<number, number[]> = {}
  const fastCross: Record<number, number[]> = {}
  for (let k = 0; k <= K; k += 1) {
  ...
  const xBase: number[] = new Array(baseCorners.length).fill(0)
  for (let i = 0; i < baseCorners.length; i += 1) {
    let sum1 = 0
    for (let k = 0; k <= K; k += 1) {
...
function computePbar(x: number, noteSeq: NoteEntry[], lnRep: LnSparseRepresentation, anchor: number[], baseCorners: number[]): Float64Array {
  const streamBooster = (delta: number): number => {
  ...
  const pStep: number[] = new Array(baseCorners.length).fill(0)
...
function computeAbar(K: number, activeColumns: number[][], deltaKs: Record<number, number[]>, ACorners: number[], baseCorners: number[]): Float64Array {
  const dks: Record<number, number[]> = {}
  ...
  const aStep: number[] = new Array(ACorners.length).fill(1)
...
function computeRbar(K: number, x: number, noteSeqByColumn: NoteEntry[][], tailSeq: NoteEntry[], baseCorners: number[]): Float64Array {
  const RStep: number[] = new Array(baseCorners.length).fill(0)

  const timesByColumn: Record<number, number[]> = {}
  for (let i = 0; i < K; i += 1) {
...
function computeCAndKs(K: number, noteSeq: NoteEntry[], keyUsage: Record<number, boolean[]>, baseCorners: number[]): { CStep: Float64Array; KsStep: Float64Array } {
...
function applyProximityEnvelope(allCorners: number[], DAll: number[], noteSeq: NoteEntry[]): number[] {
  if (!noteSeq.length) {
...
function smoothDForGraph(allCorners: number[], DAll: number[], noteSeq: NoteEntry[]): number[] {
...
export function calculate(osuText: string, speedRate: number = 1.0, odFlag: string | number | null = null, cvtFlag: string | null = null, options: SunnyCalculateOptions = {}): ReworkResult {
  ...
  const DAll: number[] = new Array(allCorners.length).fill(0)
  for (let i = 0; i < allCorners.length; i += 1) {
  ...
  const gaps: number[] = new Array(allCorners.length).fill(0)
  ...
  const cumWeights: number[] = new Array(wSorted.length).fill(0)
  let running = 0
